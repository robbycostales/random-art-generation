...
# it is not weighted by how much a color appears. I like the randomness it
# creates in the art.
...
class Art:
    ...
    # this function draws a square with the color, size, and starting position
    # (x,y) specified.
    ...
    # this function draws a rectangle with the color, size, and starting
    # position (x,y) specified.
    ...
    def draw_art(self):
        ...
        if self.mode[1] == 2:  # mode[1] = 2 draw all shapes
            for i in range(150):
                ...
                if num == 0:
                    self.draw_rhombus(
                        self.color_list[color], x_bound, y_bound, size1, size2, angle)
                if num == 1:
                    self.draw_rectangle(
                        self.color_list[color], x_bound, y_bound, size1, size2)
                if num == 2:
                    self.draw_square(
                        self.color_list[color], x_bound, y_bound, size2)
...
# these functions reset and hide all of the turtles so that there are no
# left over images from before.
def reset():
    art1.t.reset()
    art2.t.reset()
    art3.t.reset()
...
# these function make the draw_art method callable by the built in turtle
# function onkeypress.
...
# this creates an html file which opens the .svg image. However most
# browsers can open .svg on their own.
...
def run_art_generation():
    ...
    # draws all shapes in a rectangular frame
    wn.onkeypress(draw_art1, key='1')

    # draws all shapes in a circular frame
    wn.onkeypress(draw_art2, key="2")

    # draws rhombuses in a rectangular frame
    wn.onkeypress(draw_art3, key="3")
    # draws rhombuses in a circular frame
    wn.onkeypress(draw_art4, key="4")

    # draws rectangles in a rectangular frame
    wn.onkeypress(draw_art5, key="5")

    # draws rectangles in a circular frame
    wn.onkeypress(draw_art6, key="6")

    # exits the art window and closes program
    wn.onkeypress(exit, key="Escape")

    # saves the current art piece as a .svg file.
